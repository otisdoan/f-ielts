...
async function checkDatabase() {
    ...
    try {
      ...
      if (error) {
        if (error.message.includes('does not exist')) {
          console.log(`❌ ${table.name.padEnd(20)} - NOT CREATED (${table.description})`);
        } else {
  ...
  console.log('\n📚 Next steps:');
  console.log('   1. If any table shows ❌, run the migration in Supabase Dashboard');
  console.log('   2. See DATABASE_SETUP.md for detailed instructions');
  console.log('   3. For listening_tests, run: supabase/migrations/create_listening_tests.sql\n');
}
