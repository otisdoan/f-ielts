...
create index if not exists idx_speaking_topics_part on public.speaking_topics(part);
create index if not exists idx_speaking_topics_difficulty on public.speaking_topics(difficulty);
create index if not exists idx_speaking_topics_status on public.speaking_topics(status);
create index if not exists idx_speaking_topics_created_at on public.speaking_topics(created_at desc);
...
-- Insert sample data
insert into public.speaking_topics (part, title, description, difficulty, category, preparation_time, speaking_time, tips, status) values
(1, 'Work and Studies', 'Let''s talk about what you do. Do you work or are you a student? What do you enjoy most about your work/studies?', 'Easy', 'Personal', 0, 180, array['Be natural and honest', 'Use present tenses', 'Give reasons for your answers'], 'published'),
