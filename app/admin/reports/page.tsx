...
export default async function AdminReportsPage() {
  ...
  const [
    ...
    supabase.from('profiles').select('*', { count: 'exact', head: true }).eq('role', 'student'), // simple proxy for active students
  ]);
  ...
  return (
    <div className="flex-1 flex flex-col gap-6 p-8 max-w-[1440px] mx-auto w-full">
      {/* PageHeading */}
      <div className="flex flex-wrap justify-between items-end gap-3 bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex flex-col gap-1">
          <p className="text-[#181111] text-3xl font-black leading-tight tracking-tight">Analytics &amp; Reports</p>
          <p className="text-[#896161] text-sm font-normal">Detailed performance insights across all user cohorts.</p>
        </div>
        <div className="flex gap-2">
          ...
          <button className="flex items-center gap-2 px-4 py-2 bg-primary/10 text-primary rounded-lg text-sm font-bold hover:bg-primary/20 transition-colors cursor-pointer">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <div className="flex flex-col gap-2 rounded-xl p-5 border border-gray-200 bg-white shadow-sm">
          <p className="text-gray-500 text-xs font-medium uppercase tracking-wider">Total Active Users</p>
          <div className="flex items-baseline gap-2">
            <p className="text-[#181111] tracking-tight text-2xl font-black">{totalUsers.toLocaleString()}</p>
        ...
        <div className="flex flex-col gap-2 rounded-xl p-5 border border-gray-200 bg-white shadow-sm">
          <p className="text-gray-500 text-xs font-medium uppercase tracking-wider">Active Student Sessions</p>
          <div className="flex items-baseline gap-2">
            <p className="text-[#181111] tracking-tight text-2xl font-black">{activeStudents.toLocaleString()}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          ...
          <div className="h-64 bg-slate-50 flex items-center justify-center rounded-lg border border-dashed border-slate-300">
            <span className="text-slate-400 font-medium">Coming Soon - Real-time charting feature</span>
          </div>
        </div>
        {/* Pie Chart Area */}
