...
export default function EditSpeakingTopicPage({
  ...
  const loadTopic = async () => {
    setLoading(true);
    try {
      const topic = await SpeakingService.getTopicById(id);
      if (topic) {
        setFormData({
          part: topic.part,
          title: topic.title,
          ...
          preparation_time: topic.preparation_time || 60,
          speaking_time: topic.speaking_time || 120,
          tips: topic.tips && topic.tips.length > 0 ? topic.tips : [""],
          sample_answer: topic.sample_answer || "",
          status: topic.status,
        });
      } else {
        showToast("Topic not found.", "error");
        setTimeout(() => router.push("/admin/speaking"), 2000);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await SpeakingService.updateTopic(id, {
        part: formData.part,
      ...
      setTimeout(() => {
        router.push("/admin/speaking?updated=true");
      }, 1000);
    } catch (error) {
